...
const float radius_bench_with = v_max*v_max/(2*a_vmax_with)+0.4;//与工作台碰撞半径(带物品
const float radius_bench_without = v_max*v_max / (2 * a_vmax_without) + 0.4;//与工作台碰撞半径(不带物品
const float radius_robot = 1.5; //与机器人碰撞半径
const float radius_wall = 0.53; //与墙壁碰撞半径
const int maxK = 50;         //工作台最大数量
float dt = 0.02;            //采样时间
float predict_time = 0.18 ;   //轨迹推算时间长度
float v_sample = 0.1, w_sample = a_vmax_without*0.02*0.03; //采样分辨率
float alpha = 0.15,bta = 1.0, gamma = 1.0; //轨迹评价函数系数
...
int material_need_value[10];//计算原材料被需要程度
int material_need[10];     //记录原材料被需要数量
//int material_target[10];   //记录原材料送达数量
